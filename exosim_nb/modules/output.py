...
def run(opt, channel, planet):
  ...
  #add jitterless or jitterful
#  out_path = out_path[0:-1]+'--%s/'%(opt.noise_group)
...
#    hdu.header['NDR_time']= (channel[key].CDStime.item(), 'NDR time for NDR2')
...
#      hdu = fits.ImageHDU(channel[key].noise[..., i].astype(np.float32), name = 'NOISE')
      hdu = fits.ImageHDU(channel[key].noise[..., i], name = 'NOISE')
    ...
    file.write('\Diffuse:  %s'%(bool(opt.diff)))    
