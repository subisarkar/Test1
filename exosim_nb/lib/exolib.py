...
def Psf_Interp(zfile, delta_pix, WavRange):
    ...
    print "LLLL"
    print num_pix_y, num_pix_x, len(hdulist), delta_pix
...
def Psf_photometer(zfile, delta_pix, WavRange):
    ''' 
    PSF Interpolation
    Parametes
    ---------
...
def pointing_jitter(jitter_file, total_observing_time, frame_time, opt, rms=None):
...
#  psd_freq = psd_freq[1:]
  ...
  number_of_samples_ = np.int(osf*np.ceil(total_observing_time/frame_time).simplified)  + 100
  N0 = number_of_samples_
#  
#  DF = psd_freq[1]-psd_freq[0]
#  FS = osf/ frame_time.rescale(pq.s).magnitude
#  number_of_samples_ = np.int(np.round(FS/DF))
#  number_of_samples_ = number_of_samples_*4
###

  number_of_samples = 2**(np.ceil(np.log2(number_of_samples_/2+1)))
  number_of_samples = number_of_samples_/2+1  
...
#  number_of_samples = rnd + np.ceil(number_of_samples/ rnd) * rnd
  ...
  freq = np.linspace(0.0, 0.5* osf/ frame_time.rescale(pq.s).magnitude, number_of_samples)

#  print freq[1]-freq[0], DF, psd_freq


  print "NEW FREQ MAX", freq.max()
  ...
  print "interpolation of PSD done"
#  import matplotlib.pyplot as plt
#  plt.figure(33)
...
#    pitch_jit *= np.sqrt(norm)
  ...
  print len(yaw_jit), N0
  print "jitter RMS in mas", np.std (yaw_jit)*3600*1000, np.std(pitch_jit)*3600*1000
